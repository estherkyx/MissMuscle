...
import { CRITERIA, type SavedCorrection } from './findings';
import { createReplayResources } from './replay-resources';

function ReplayPlayer({ recording, seekSec }: { recording: RecordingWindow; seekSec: { time: number; nonce: number } }) {
  ...
  useEffect(() => { seek(recording.startSec + seekSec.time); }, [recording, seekSec]); // eslint-free: seeking is driven by explicit review selection.
...
export function SessionReplay({ recent, corrections }: { recent: RecordingWindow; corrections: SavedCorrection[] }) {
  ...
  const [seek, setSeek] = useState({ time: 0, nonce: 0 });
  ...
  return <section className="post-session" aria-label="Live session review">
    ...
    <div className="replay-tabs"><button className={selected === -1 ? 'small-button' : 'secondary small-button'} onClick={() => { setSelected(-1); setSeek({ time: 0, nonce: seek.nonce + 1 }); }}>Latest 10 seconds</button>{corrections.map((item, index) => <button key={item.criterionId} className={selected === index ? 'small-button' : 'secondary small-button'} onClick={() => { setSelected(index); setSeek({ time: item.evidence[0]?.timestampSec ?? 0, nonce: seek.nonce + 1 }); }}>{CRITERIA[item.criterionId]}</button>)}</div>
    <ReplayPlayer key={recording.id} recording={recording} seekSec={seek} />
    {correction && <div className="review-evidence"><h3>{CRITERIA[correction.criterionId]}</h3><p>{correction.result.report.formChecks?.find(item => item.criterionId === correction.criterionId)?.note}</p><div className="evidence-actions">{correction.evidence.map((item, index) => <button key={index} className="secondary small-button" onClick={() => setSeek({ time: item.timestampSec, nonce: seek.nonce + 1 })}>Show evidence · {item.timestampSec.toFixed(2)}s</button>)}</div></div>}
  </section>;
}
