...
export function LiveExercise({ visible }: { visible: boolean }) {
  ...
  return <section className="video-workspace live-workspace" hidden={!visible} aria-label="Live exercise coach">
    ...
    {!active && phase !== 'ending' && !review && <div className="live-introduction"><h3>Make room for your next rep.</h3><p>Position your laptop so your shoulders, elbows and wrists stay visible. Start with one person in view.</p><p>Your body map follows your movement. Your coach reviews recent reps, automatically speaks useful corrections as new analysis arrives. Just exercise and listen; there is no need to speak back.</p><p className="capture-notice">Starting enables your camera and automatic coach audio. No microphone access is requested. Recent video and up to five correction clips stay temporarily on this device; sampled frames are sent for AI analysis. Replay clips contain video only.</p><span className="muted">Laptop Chrome · live body mapping + periodic AI feedback</span></div>}
    <div hidden={!active}>
