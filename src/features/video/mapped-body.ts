...
export function drawMappedBody(ctx: CanvasRenderingContext2D, width: number, height: number, videoWidth: number, videoHeight: number, poses: Landmark[][]) {
  ...
  const visible = (...indices: number[]) => indices.every(index => isVisibleJoint(pose[index]));
  const size = visible(11, 12) ? Math.max(12, Math.min(width * 0.09, Math.hypot(point(11).x - point(12).x, point(11).y - point(12).y) * 0.21)) : width * 0.045;
  ...
  for (const [a, b] of [[11, 13], [12, 14]]) muscle(a, b, '#ed383e', size * 0.78);
