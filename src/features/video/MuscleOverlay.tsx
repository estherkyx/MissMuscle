    ...
    function tick(now: number) {
      ...
      try {
        if (video!.seeking || video!.readyState < 2) { clear(); setMapped(false); setMotion(null); sync.reset(); }
          ...
          setStatus(result.landmarks.length > 1 ? 'Multiple people visible. Use a clip with one person.' : hasPose ? 'Mapped to the current video frame' : 'No clear body regions. Keep your arms and torso in view.');
        }
